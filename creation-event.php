...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty(trim($_POST["event_date"]))) {
        $event_date_err = "Veuillez entrer une date pour fête.";
    } else {
        $event_date = trim($_POST["event_date"]);
    }

    if (empty(trim($_POST["location"]))) {
        $location_err = "Veuillez entrer un lieu pour fête.";
    } else {
    ...
    <div class="container">
        ...
        <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post" enctype="multipart/form-data">
            ...
            <div class="container_submit_button">
                <button class="custom_button" type="submit">Créer fête</button>
            </div>
        </form>
    </div>
